...
func (_q *DepartmentQuery) QueryAdminUsers() *AdminUserQuery {
...
func (_q *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withAdminUsers: _q.withAdminUsers.Clone(),
...
func (_q *DepartmentQuery) WithAdminUsers(opts ...func(*AdminUserQuery)) *DepartmentQuery {
	query := (&AdminUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAdminUsers = query
	return _q
}
...
func (_q *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := _q.withAdminUsers; query != nil {
		if err := _q.loadAdminUsers(ctx, query, nodes,
...
func (_q *DepartmentQuery) loadAdminUsers(ctx context.Context, query *AdminUserQuery, nodes []*Department, init func(*Department), assign func(*Department, *AdminUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adminuser.FieldDeptID)
	}
